...
baseline_algorithmlist=["es/global_deg_min_0.0000", "tihs", 
               "ns/global_deg_1.0000", 
               "rw/rw_c_1", "ff/ff_c_0.51_0.20", "ns/global_deg_0.0000"]
compete_algorithmlist = ["es/global_deg_min_0.0000",  "tihs",
              "ff/ff_c_0.51_0.20", "ns/global_deg_0.0000", "ns/global_deg_1.0000",
...
evallist = ["degree", "intersect", "pairdeg", "size", "Time", 
            "singular_value", "size_wcc", "global_cc_norm",
            "density_norm", "overlapness_norm", "effective_diameter_norm"]
    ...
    for algoname in algo2evaldict.keys():
        ...
        if evaldict == -1:
            ...
            with open(outputname, "a") as f:
                line = ",".join([algorithm, algo_opt, eval_opt] + [ str(evaldict[ename]) for ename in evallist])
                f.write(line + "\n")
...
def aggregate_all_datasets(portion_str, type):
	...
	for didx, dataname in enumerate(dataset):
		...
		for i, row in d.iterrows():
			...
			for idx, col in enumerate(norm_columns):
				algoname2normevallist[algoname][col].append(row[col])
	...
	for algoname in algoname2normevallist.keys():
		...
		with open(norm_outputname, "a") as f:
			line = ",".join([algorithm, algo_opt, eval_opt] + [str(algoname2normevallist[algoname][e]) for e in columns])
			f.write(line + "\n")
    ...
    d.to_csv(outputname, index=False)
...
def aggregate_all_portions(type):
	...
	label_col = {'degree': 'Degree',
	 ...
	 'effective_diameter': 'Eff. Diam.',
	 'singular_value': 'SV'
	}
	# D-Stat Output
	dstat_outputname = "./csvs/allportion_" + type + "_agg_dstat.csv"
	with open(ranking_outputname, "w") as f:
		label_columns = [label_col[c] for c in evallist]
		line = ",".join(label_columns)
		f.write(line + "\n")
	for algoname in algo2dstat_prop.keys():
			...
			if evaldict == -1:
				...
				with open(dstat_outputname, "a") as f:
					line = ",".join([ str(evaldict[ename]) for ename in evallist])
					f.write(line + "\n")

	# Rank Output
	label_col['avg'] = 'Avg. Ranking'
	ranking_outputname = "./csvs/allportion_" + type + "_agg_rank.csv"
	with open(ranking_outputname, "w") as f:
		columns = evallist + ["avg"]
		label_columns = [label_col[c] for c in columns]
		line = ",".join(label_columns)
		f.write(line + "\n")
	for algoname in algo2rank_prop.keys():
			...
			if evaldict == -1:
				...
				with open(ranking_outputname, "a") as f:
					line = ",".join([ str(evaldict[ename]) for ename in evallist + ["avg"]])
					f.write(line + "\n")
					
	# Norm Output
	label_col['avg'] = 'Avg. Z-Score'
...
if __name__ == "__main__":
	...
	parser.add_argument('--type', required=True, default='baseline', type='str')
