...
double AlgorithmMH::get_acceptance_rate(double delta, double k){
    double prob;
    if (k < 0){
        if (delta > 0){
            prob = 1.0;
...
void AlgorithmMH::step_log(string postfix, bool accept, HSet *sampled, double delta, double prob){
...
void AlgorithmMH::initiate(HSet *sampled){
    inhedges.clear();
    outhedges.clear();
    vector<int> tmp;
...
int AlgorithmMH::sample_he(HSet *sampled, string opt){
    int sampled_hedge = -1;
...
void AlgorithmMH::update_partial(set<int> &hset, vector<int> &hedges, string opt){
    if (opt.compare("add") == 0){
        for (auto h : hset){
            hedges.push_back(h);
        }
...
void AlgorithmMH::update(set<int> &add_set, set<int> &rm_set, HSet *sampled){
...
// Run
HSet* AlgorithmMH :: run(int turn, double target_portion){
